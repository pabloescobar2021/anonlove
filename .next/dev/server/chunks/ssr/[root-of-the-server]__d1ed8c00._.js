...
function useSetAnon() {
    ...
    const checkAnon = async (from_user_id, to_user_id)=>{
        try {
            setLoading(true);
            setError(null);
            const { data: toUser, error: toUserError } = await __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$supabase$2f$alSupabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from("users").select("id_user").eq("public_id", to_user_id).single();
            if (toUserError || !toUser) {
                throw new Error("Пользователь не найден, кись");
            }
            const { data, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$supabase$2f$alSupabase$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["supabase"].from("user_anonymous_settings").select("is_anon, anon_used_once").eq("from_user_id", from_user_id).eq("to_user_id", toUser.id_user).single();
            if (error) throw error;
            return {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d1ed8c00._.js.map